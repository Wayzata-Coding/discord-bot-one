...
client.on("ready", () => {
  ...
  client.user.setActivity(
    `s!help | ${client.guilds.cache.size} servers, ${client.users.cache.size} members`,
    { type: "PLAYING" }
  );
});
...
client.on("message", (message) => {
  ...
  try {
    // console.log(client)
    if (commander.guildOnly && !message.guild)
...
if (process.env.BOT_TOKEN) {
  configToken = process.env.BOT_TOKEN.toString();
} else {
...
var configToken =
  "MTAyOTkyNDIyOTUxMDA4MjczMQ.GPmrLF.K46CunldRXAeTbhBXuwMqSu7NWPaKHgfNG40Oo";
